...
namespace Paulov.TarkovServices.Helpers
{
    public static class DatabaseHelpers
    {
        ...
        {
            languages = new();
            var db = DatabaseService.GetDatabaseProvider();
            if (db == null)
                return false;
        ...
        public static IEnumerable<KeyValuePair<string, JToken>> LoadDatabaseFileAsEnumerable(string databaseFilePath)
        {
            ...
            EntryModel entry = DatabaseService.GetDatabaseProvider().Entries.FirstOrDefault(x => x.FullName == filePath);
        ...
        out JObject locations)
        {
            ...
            var locationEntries = DatabaseService.GetDatabaseProvider().Entries.Where(x => x.FullName.StartsWith("database/locations/")).ToArray();
        ...
        out JObject traders)
        {
            ...
            var entries = DatabaseService.GetDatabaseProvider().Entries.Where(x => x.FullName.StartsWith("database/traders/"));
